function detect_ripples_11062017BG(session,refs)
%--------------------------------------------------------------------------
...
time_b = timeStamps(bins + bindur/2);
...
ripple.states = zeros(size(ripples, 1),1);
...
%% Find indices of ripple start and stop times in origninal time vector
[~,ripple.startIdx] = ismember(ripples(:,1),timeStamps);
[~,ripple.stopIdx] = ismember(ripples(:,2),timeStamps);
%% ALIGN RIPPLES
...
save(['Ripples', filename, '.mat'], 'ripples','LFPbp','LFP','timeStamps','time_b',...
