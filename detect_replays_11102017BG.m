function detect_replays_11102017BG(targetBins)%(session,refs)
...
mincells = 2; %Temporary change to see what happens in analysis
...
[Sequences,r_high,r_low, numCellsInSeq] = candidateSpindleReplays_11102017(spikeData,targetBins,mincells);     % Get all candidate replays
Seq_cand = Sequences;
...
Rs = zeros(numSeq,1);                                                       % Matrix with the correlations (for first spike, mean spike time, median spike)
Ps = zeros(numSeq,1);                                                       % Matrix with the p-values
highCorrSeq = cell(numSeq,1);
for r = 1:numSeq                                                            % For each replay, get the correlation of the sequence with...
    [rs ,p] = corr(Seq_cand{r}(:,seq_crit), forwardSeq{r}, 'type','Spearman'); % ...forward sorted cell numbers
...
placeCellIsi = cells(numCells,1);

for i = 1:numCells
    placeCellIsi{i} = diff(spikeData{i});
end

%% Alternate Permutations Via ISI Randomization
randperm(length(cells1sort))
...
Rperm = zeros(numSeq,repet);                                             % Matrix with the forward correlations
...
for r = 1:numSeq                                                            % For each replay
    for i = 1:repet                                                         % For each permutation
        Rperm(r,i) = corr(Seq_perm{r}(:,i),highCorrSeq{r}, 'type','Spearman'); % Get the correlation of shuffled and the forward sequence
...
for r = 1:numSeq                                                            % For each replay
%     %Forward replay
    Rless = sum(Rperm(r,:) <= Rs(r,1));                               % Find how many permutations of the replay have less correlation with the prototype sequence than the original sequence
    Rless = Rless/repet*100;                                          % Turn it to percentage
    Rmore = sum(Rperm(r,:) >= Rs(r,1));                                  % Same for permutations with higher correlation than the original
